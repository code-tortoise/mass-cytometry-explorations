...
target_dir <- "~/mass-cytometry-explorations/notebooks"

# Detect current working directory
current_dir <- getwd()

# If current directory is NOT the target, switch to the target
if (!identical(current_dir, target_dir)) {
    setwd(target_dir)
}
...
images <- JacksonFischer_2020_BreastCancer(
    ...
    full_dataset = FALSE,   # loads 125 images instead of 375
    cohort = "Basel",
    force = "TRUE"
)
images
